...
function solve_mn_mc_mld_simple_water(data::Union{Dict{String,<:Any},String}, model_type::Type, solver; kwargs...)
    return solve_mc_model(data, model_type, solver, build_mn_mc_mld_simple_flexible_loads; multinetwork=true, kwargs...)
end

"Multinetwork load shedding problem including storage"
function build_mn_mc_mld_simple_flexible_loads(pm::AbstractUnbalancedPowerModel)
...
function build_mn_mc_mld_simple_flexible_loads(pm::AbstractUBFModels)
    for (n, network) in nws(pm)
...
"Multinetwork load shedding problem including storage"
function build_mn_mc_mld_multi_scenario_flexible_loads(pm::AbstractUnbalancedPowerModel)
    for (n, network) in nws(pm)
        variable_mc_branch_power(pm; nw=n)
        variable_mc_switch_power(pm; nw=n)
        variable_mc_transformer_power(pm; nw=n)
    ...
    network_ids = sort(collect(nw_ids(pm)))
    int_len = pm.data["interval_len"]
    m_sc = Int(length(network_ids)/int_len)
    for sc in 0:m_sc-1
        n_0 = network_ids[sc*int_len + 1]
        n_1 = network_ids[sc*int_len + 1]
        ...
        for n_2 in network_ids[sc*int_len+2 : (sc+1)*int_len]
            for i in ids(pm, :storage; nw=n_2)
                constraint_storage_state(pm, i, n_1, n_2)
            end
